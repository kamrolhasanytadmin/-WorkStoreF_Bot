...
texts = {'en': {'buy_btn': '🛒 Buy',
 ...
 'bn': {'buy_btn': '🛒 কিনুন',
        ...
        'buy_ask_hotmail': '🛒 সটোর > Hotmail',
        'buy_ask_outlook': '🛒 সটোর > Outlook',
        'buy_ask_outlook_fr': '🛒 সটোর > Outlook.fr',
...
def get_admin_menu():
    ...
    markup.add(
        ...
        KeyboardButton('💰 Manage Balance'), KeyboardButton('⚙ Payment Settings'),
...
def is_back_or_cancel_text(text):
    ...
    return any(key in normalized for key in ("cancel", "back", "বাতিল", "ব্যাক", "বযাক"))
...
def send_welcome(message):
    ...
    if not user:
        ...
        bot.send_message(
            message.chat.id, 
            "Please select your language 👇\nঅনগরহ করে আপনার ভাষা নিরবাচন করন 👇", 
            reply_markup=markup
        )
    else:
...
def handle_menu(message):
    ...
    if user_id in ADMIN_IDS:
        if text == '📊 Bot Stats':
            ...
            markup.add(KeyboardButton(' View Stock'), KeyboardButton('🗑 Clear Stock'))
            ...
            markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
            markup.add(KeyboardButton('🗑 Clear VPN'), KeyboardButton('🗑 Clear Hotmail'))
            markup.add(KeyboardButton('🗑 Clear Outlook'), KeyboardButton('🗑 Clear Outlook.fr'))
    ...
    # Handle main menu commands
        ...
        markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
        markup.add(KeyboardButton('English 🇬🇧'), KeyboardButton('বাংলা 🇧🇩'))
        back_text = '🔙 Back' if lang == 'en' else '🔙 বযাক'
    ...
    if message.document:
        try:
            ...
            try:
                ...
                raw_text = downloaded_file.decode('ISO-8859-1')
        except Exception as e:
...
def process_deposit_amount(message, method):
    ...
    if text in [' Cancel', ' বাতিল (Cancel)', '🔙 Back', '🔙 বযাক'] or text in texts['en'].values() or text in texts['bn'].values():
...
def handle_buy_item(call):
    ...
    conf = get_config()
    if cat == "Hotmail": price = conf.get("hotmail_price", 5.0)
    elif cat == "Outlook": price = conf.get("outlook_price", 5.0)
    elif cat == "Outlook.fr": price = conf.get("outlook_fr_price", 5.0)
    else: price = conf.get("vpn_price", 15.0)
    
    # VPN remains 1-by-1
    if cat not in ["Hotmail", "Outlook", "Outlook.fr"]:
        user = users_collection.find_one({"chat_id": user_id}, {"balance": 1})
        balance = user.get("balance", 0.0) if user else 0.0
        ...
        # Mail Accounts Multi-Buy
        stock = vpn_collection.count_documents({"category": cat, "duration": dur, "status": "available"})
        if stock == 0:
            bot.answer_callback_query(call.id, texts[lang]['buy_sold_out'], show_alert=True)
            return
            
        bot.answer_callback_query(call.id)
        msg_text = f"🛒 **{cat}**\n\nHow many accounts do you want to buy? (কয়টি অযাকাউনট কিনতে চান?)\n📦 Available Stock: {stock}\n💵 Price per account: {price} ৳\n\n*(Please type a number / শধ সংখযা লিখন)*"
        
        bot.send_message(user_id, msg_text, reply_markup=get_cancel_menu(lang), parse_mode="Markdown")
        bot.register_next_step_handler_by_chat_id(user_id, process_buy_quantity, cat, dur, price, stock)

def process_buy_quantity(message, cat, dur, price, stock):
    user_id = message.chat.id
    lang = get_user_lang(user_id)
    
    if message.text in [' Cancel', ' বাতিল (Cancel)', '🔙 Back', '🔙 বযাক'] or message.text in texts['en'].values() or message.text in texts['bn'].values():
    ...
    try:
        ...
        bot.send_message(user_id, " Invalid quantity. Please enter a valid number. (সঠিক সংখযা লিখন)", reply_markup=get_main_menu(lang, user_id))
        return
    ...
    if balance < total_price:
        bot.send_message(user_id, f" Insufficient balance! (পরযাপত বযালেনস নেই)\n\nTotal Price: {total_price} ৳\nYour Balance: {balance} ৳", reply_markup=get_main_menu(lang, user_id))
        return
        
    # Ask Delivery Format
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        InlineKeyboardButton(" Text Message", callback_data=f"deliv_{cat}_{dur}_{qty}_text"),
        InlineKeyboardButton("📄 .txt File", callback_data=f"deliv_{cat}_{dur}_{qty}_txt"),
        InlineKeyboardButton("📊 Excel (.csv) File", callback_data=f"deliv_{cat}_{dur}_{qty}_csv"),
        InlineKeyboardButton(" Cancel", callback_data="confbuy_no")
    )
    
    bot.send_message(user_id, f"🛒 **Order Summary**\n\nItem: **{cat}**\nQuantity: **{qty}**\nTotal Price: **{total_price} ৳**\n\n📥 **Choose delivery format / ফাইল ফরমযাট সিলেকট করন:**", reply_markup=markup, parse_mode="Markdown")

@bot.callback_query_handler(func=lambda call: call.data.startswith('deliv_'))
def handle_delivery_choice(call):
    ...
    markup = InlineKeyboardMarkup()
    markup.add(
        InlineKeyboardButton(texts[lang]['buy_yes'], callback_data=f"confmulti_{cat}_{dur}_{qty}_{method}"),
        InlineKeyboardButton(texts[lang]['buy_no'], callback_data="confbuy_no")
    )
    
    method_str = "Text Message" if method == 'text' else (".txt File" if method == 'txt' else "Excel (.csv) File")
    
    bot.edit_message_text(f"⚠ **Confirm Purchase**\n\nItem: {cat}\nQuantity: {qty}\nFormat: {method_str}\nTotal Price: {total_price} ৳\n\nAre you sure?", chat_id=user_id, message_id=call.message.message_id, reply_markup=markup, parse_mode="Markdown")

@bot.callback_query_handler(func=lambda call: call.data.startswith('confmulti_'))
def handle_confirm_buy_multi(call):
    parts = call.data.split('_')
    cat = parts[1]
    dur = parts[2]
    qty = int(parts[3])
    ...
    # Fetch exact quantity of available items
    items = list(vpn_collection.find({"category": cat, "duration": dur, "status": "available"}).limit(qty))
    if len(items) < qty:
    ...
    # Prepare Credentials exactly as they were added
    creds_list = [item.get("credentials", "") for item in items]
    raw_creds_text = "\n".join(creds_list)
    
    if method == 'text' and len(raw_creds_text) > 3500:
        method = 'txt' # Fallback if text is too long for telegram
        
    success_msg = f"✅ **Purchase Successful!**\n\n**Item:** {cat} (x{qty})\n**Total Paid:** {total_price} ৳\n**New Balance:** {new_balance} ৳\n\n"
    
    if method == 'text':
        bot.edit_message_text(f"{success_msg}**Accounts:**\n\n`{raw_creds_text}`", chat_id=user_id, message_id=call.message.message_id, parse_mode="Markdown")
    else:
        bot.edit_message_text(f"{success_msg}*(File is being uploaded...)*", chat_id=user_id, message_id=call.message.message_id, parse_mode="Markdown")
        
        file_data = io.BytesIO(raw_creds_text.encode('utf-8'))
        if method == 'txt':
            file_data.name = f"{cat}_{qty}_accounts.txt"
        else: # csv for excel
            file_data.name = f"{cat}_{qty}_accounts.csv"
